...
public class FeedForwardAndBackPropagation {
    ...
    private static List<LoadData.Instance> dataSet; //list containing 1 row of training example
    ...
    public static void main(String[] args) throws java.io.IOException {
        ...
        dataSet = loadData.loadData(file, true);
        ...
        System.out.println("Accuracy tested on training dataset: " + test.calcAccuracyOfModel(dataSet) + " %");
        /*System.out.println("\nBiases and Weights: \n");
    ...
    private FeedForwardAndBackPropagation(int... NETWORK_LAYER_SIZES) {
        ...
        for (int i = 0; i < NETWORK_SIZE; i++) {
            ...
            if (i > 0) {
                ...
                //the weight in the ith layer is an array of arrays where the first array is the number of neurons in the layer, the second is the number of neurons in the previous layer
    ...
    private double[] feedForward(double... input) {
        ...
        if (input.length != this.INPUT_SIZE) {
            System.out.println("The number of neurons in the input layer are not equal to the number of input variables.");
            return null;
        }
    ...
    private double MSE(List<LoadData.Instance> instances) {
        double v = 0;
        for (int i = 0; i < instances.size(); i++) {
            v += MSE(instances.get(i).inputVariables, instances.get(i).classValues);
        }
    ...
    private void train(List<LoadData.Instance> instances, int iterations, double learning_rate) {
        for (int iteration = 0; iteration < iterations; iteration++) {
            for (int instance = 0; instance < instances.size(); instance++) {
                double[] x = instances.get(instance).inputVariables;
                double[] y = instances.get(instance).classValues;
                this.train(x, y, learning_rate);
            }
            System.out.println("Error of the instance at this iteration: " + MSE(instances));
        }
    }

    //Training 1 training set:
